...
public class PhotosInterface {
    ...
    public void addMachineTags(String photoId, String[] tags) throws IOException, SAXException, FlickrException {
    	addTags(photoId, tags, false);
    }
    
    public void addTags(String photoId, String[] tags) throws IOException, SAXException, FlickrException {
    	addTags(photoId, tags, true);
    }
    
    public void addTags(String photoId, String[] tags, boolean quote_tags) throws IOException, SAXException, FlickrException {
        List<Parameter> parameters = new ArrayList<Parameter>();
        parameters.add(new Parameter("method", METHOD_ADD_TAGS));
        parameters.add(new Parameter("api_key", apiKey));

        parameters.add(new Parameter("photo_id", photoId));
        parameters.add(new Parameter("tags", StringUtilities.join(tags, ",", quote_tags)));
        parameters.add(
            new Parameter(
                "api_sig",
    ...
    public PhotoList search(SearchParameters params, int perPage, int page)
        ...
        String privacy_filter = params.getPrivacyFilter();
        
//        Log.e("FlickrJ", "FlickrJ privacy filter: " + privacy_filter);
        if (params.getForceAuthentication() ||
        		privacy_filter == null ||
        		(privacy_filter != null &&
        				!(privacy_filter.equals(Integer.toString(Flickr.PRIVACY_LEVEL_PUBLIC))
        		))) {

//            Log.i("FlickrJ", "I am adding the authentication.");
        ...
        /*
        Log.e("FlickrAPI", "Search parameters:");
        for (Parameter p : parameters) {
            Log.d("FlickrAPI", p.getName() + ": " + p.getValue());
        }
		*/
    ...
    public InputStream getImageAsStream(Photo photo, int size)
      throws IOException, FlickrException {
        String urlStr = "";
        if (size == Size.SizeType.SQUARE.ordinal()) {
            urlStr = photo.getSmallSquareUrl();
        } else if (size == Size.SizeType.THUMB.ordinal()) {
            urlStr = photo.getThumbnailUrl();
        } else if (size == Size.SizeType.SMALL.ordinal()) {
            urlStr = photo.getSmallUrl();
        } else if (size == Size.SizeType.MEDIUM500.ordinal()) {	// FIXME
            urlStr = photo.getMediumUrl();
        } else if (size == Size.SizeType.LARGE.ordinal()) {
            urlStr = photo.getLargeUrl();
        } else if (size == Size.SizeType.ORIGINAL.ordinal()) {
     ...
     * @throws FlickrException
     */
    public Bitmap getImage(Photo photo, int size)
      throws IOException, FlickrException {
        return BitmapFactory.decodeStream(getImageAsStream(photo, size));
    }
