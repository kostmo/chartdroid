...
public class AsyncTaxonInfoPopulator implements SemaphoreHost {
	...
	Context context;
	public AsyncTaxonInfoPopulator(Context c) {
		context = c;
...
//			Log.d(TAG, "TSN: " + ti.tsn + "; rank_id: " + krr.rank_id + "; kingdom_id: " + krr.kingdom_id);
    ...
    public class TaxonNameRetrievalTask extends AsyncTaskModified<Void, Void, String> {
    	...
    	TaxonNameRetrievalTask(ViewHolderTaxon taxon_info_holder, long tsn, FlickrPhotoDrawableManager drawable_manager, int preferred_thumbnail_index, boolean thumbnail_needed ) {
    		this.tsn = tsn;
    		this.taxon_view_holder = taxon_info_holder;
    		this.preferred_thumbnail_index = preferred_thumbnail_index;
    		this.drawable_manager = drawable_manager;
    		this.thumbnail_needed = thumbnail_needed;
    	}
...
//				Log.d(TAG, "Taxon name was cached: " + taxon_name);
	    ...
	    void set_view_elements(String taxon_name) {
	    	...
	    	if (thumbnail_needed) {
		    	if (drawable_manager != null && taxon_view_holder.thumbnail != null) {
		    		drawable_manager.fetchThumbnailImageOnThread( taxon_name, taxon_view_holder.thumbnail, this.preferred_thumbnail_index );
		    		
		    	} else {
		    		
		    		Log.e(TAG, "Crap, we can't proceed with populating the thumbnail for " + taxon_name);
		    		
		    		Log.e(TAG, "drawable_manager: " + drawable_manager);
		    		Log.e(TAG, "taxon_view_holder.thumbnail: " + taxon_view_holder.thumbnail);
		    	}
	    	}
	...
	public void fetchTaxonNameOnThread(final long tsn, final ViewHolderTaxon taxon_info_holder, FlickrPhotoDrawableManager drawable_manager, int preferred_thumbnail_index, boolean thumbnail_needed ) {
		if (taxon_name_map.containsKey(tsn)) {
			...
			if (taxon_info_holder.thumbnail != null) {
				...
				if (thumbnail_needed)
					drawable_manager.fetchThumbnailImageOnThread( taxon_name, taxon_info_holder.thumbnail, preferred_thumbnail_index );
			}
		} else {

			taxon_info_holder.taxon_name_textview.setText( "loading..." );
			new TaxonNameRetrievalTask(taxon_info_holder, tsn, drawable_manager, preferred_thumbnail_index, thumbnail_needed ).execute();
		}
	}
