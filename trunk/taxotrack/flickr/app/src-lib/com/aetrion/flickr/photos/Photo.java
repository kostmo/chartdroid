...
public class Photo {
    ...
    public void setSizes(Collection sizes) {
        ...
        while (it.hasNext()) {
            ...
            if (size.getLabel() == Size.SizeType.SMALL.ordinal()) {
                smallSize = size;
            } else if (size.getLabel() == Size.SizeType.SQUARE.ordinal()) {
                squareSize = size;
            } else if (size.getLabel() == Size.SizeType.THUMB.ordinal()) {
                thumbnailSize = size;
            } else if (size.getLabel() == Size.SizeType.MEDIUM500.ordinal()) {	// FIXME
                mediumSize = size;
            } else if (size.getLabel() == Size.SizeType.LARGE.ordinal()) {
                largeSize = size;
            } else if (size.getLabel() == Size.SizeType.ORIGINAL.ordinal()) {
                originalSize = size;
            }
        }
    }
