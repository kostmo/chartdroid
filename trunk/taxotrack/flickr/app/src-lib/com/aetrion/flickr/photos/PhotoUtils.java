...
//                note.setText(noteElement.getTextContent());
                note.setText(noteElement.getNodeValue());
                notes.add(note);
            }
        ...
        try {
            ...
            String machineTagsAttr = photoElement.getAttribute("machine_tags");
            ...
            // Karl
            } else if (!machineTagsAttr.equals("")) {
            	
                String[] values = machineTagsAttr.split("\\s+");
                for (int i = 0; i < values.length; i++) {
                    Tag tag = new Tag();
                    tag.setIsMachineTag(true);
                    tag.setValue(values[i]);
                    tags.add(tag);
                }
